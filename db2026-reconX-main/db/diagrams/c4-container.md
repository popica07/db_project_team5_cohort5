# TICKET-ADV003 — C4 Container diagram (Level 2)

> Zoom into the ReconX system boundary. Seven independently deployable
> containers (one box per process in `docker compose up`) plus the external
> actors/systems carried over from the Context diagram, sitting outside the
> boundary.
